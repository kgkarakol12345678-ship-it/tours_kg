...
const typeConfig = {
    ...
    // Специальный тип для пустых/неизвестных
...
async function fetchAirtableData() {
    try {
        ...
        allRecords = data.records.map(record => {
            ...
            // Если тип есть – обрезаем пробелы, иначе присваиваем null
            if (fields.Type) {
                fields.Type = fields.Type.trim();
            } else {
                fields.Type = null; // явно обозначим отсутствие типа
            }
        ...
        // Собираем уникальные непустые типы (null не включаем)
        const typesSet = new Set();
        allRecords.forEach(item => {
            if (item.Type) typesSet.add(item.Type);
        });
        // Добавляем специальный тип "Uncategorized" если есть записи без типа
...
function renderPlaces(places) {
    ...
    const html = places.map(place => {
        // Определяем тип: если Type отсутствует, используем 'Uncategorized'
