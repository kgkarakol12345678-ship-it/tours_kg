...
let uniqueTypes = [];           // уникальные типы (для фильтров)

// Соответствие типа -> иконка и русская подпись (можно расширять)
const typeConfig = {
    ...
    Nature:      { label: 'Природа',      icon: 'fa-tree' }   // на всякий случай
    // Для любого другого типа будет использоваться иконка по умолчанию (fa-map-pin)
};
...
async function fetchAirtableData() {
    try {
        ...
        // Преобразуем записи и сразу чистим тип от пробелов
        allRecords = data.records.map(record => {
            const fields = record.fields;
            // Если тип есть – обрезаем пробелы, иначе null
            if (fields.Type) fields.Type = fields.Type.trim();
            return { id: record.id, ...fields };
        });

        // Собираем уникальные непустые типы
...
// Создание кнопок фильтров на основе uniqueTypes
function renderFilterButtons() {
    // Всегда добавляем кнопку "Все" первой
    ...
    // Назначаем обработчики на все новые кнопки
...
function renderPlaces(places) {
    ...
    const html = places.map(place => {
        ...
        // Фото: поле Photo (Attachment) -> берём первый элемент
